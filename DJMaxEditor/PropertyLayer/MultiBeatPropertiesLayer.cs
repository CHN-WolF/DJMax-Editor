...
{
    /// <summary>
    /// Inspector wrapper for a multi-beat-event selection. The getter returns the
    /// first event's value; the setter commits one undoable edit for the whole selection.
    /// </summary>
    public class MultiBeatPropertiesLayer
    {
        private readonly IReadOnlyList<EventData> _items;
        private readonly ChartEditController _edits;

        public MultiBeatPropertiesLayer(IReadOnlyList<EventData> items, ChartEditController edits)
        {
